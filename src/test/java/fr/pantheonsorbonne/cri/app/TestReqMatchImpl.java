...
public class TestReqMatchImpl {
    ...
    void test() {
...

        {
            ...
            ReqMatch rmatch = ReqMatch.builder().className("A").packageName("toto").args("([LString;)i").methodName("main").line(10).issue("1").build();
            Assertions.assertTrue(rmatch.isMatch(ste));
        }

        {
            StackTraceElement ste = new StackTraceElement("A.java", "toto", "A", "main", "([LString;)i'", 10);
            ReqMatch rmatch = ReqMatch.builder().className("A").packageName("toto").args("[LString").methodName("main").line(10).issue("1").build();
            Assertions.assertTrue(rmatch.isMatch(ste));
        }

        {
            StackTraceElement ste = new StackTraceElement("A.java", "toto", "A", "main", "([LString;D;I;)V'", 10);
            ReqMatch rmatch = ReqMatch.builder().className("A").packageName("toto").arg("[LString").arg("D").arg("I").methodName("main").line(10).issue("1").build();
            Assertions.assertTrue(rmatch.isMatch(ste));
        }

        {
            StackTraceElement ste = new StackTraceElement("A.java", "toto", "A", "main2", "([LString;D;I;)V'", 10);
            ReqMatch rmatch = ReqMatch.builder().className("A").packageName("toto").arg("[LString").arg("D").arg("I").methodName("main").issue("1").build();
...

        {
            ...
            StackTraceElement ste2 = new StackTraceElement("A.java", "toto", "A", "main", "([LString;D;D;)V'", 10);
            ReqMatch rmatch = ReqMatch.builder().className("A").packageName("toto").arg("[LString").arg("D").arg("I").methodName("main").issue("1").build();
