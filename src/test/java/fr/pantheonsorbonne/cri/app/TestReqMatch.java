...
public class TestReqMatch {
    ...
    void test() {
        ...
        {
            StackTraceElement ste = new StackTraceElement("A.java", "toto", "toto.A", "main", "([LString;)i'", 10);
            ReqMatch rmatch = new ReqMatcherBuilder().className("A").packageName("toto").args("[LString").methodName("main").line(10).commit("1").build();
            Assertions.assertTrue(rmatch.isMatch(ste));
        }

        {
            StackTraceElement ste = new StackTraceElement("A.java", "toto", "toto.A", "main", "([LString;D;I;)V'", 10);
            ReqMatch rmatch = new ReqMatcherBuilder().className("A").packageName("toto").arg("[LString").arg("D").arg("I").methodName("main").line(10).commit("1").build();
            Assertions.assertTrue(rmatch.isMatch(ste));
        }

        {
            StackTraceElement ste = new StackTraceElement("A.java", "toto", "toto.A", "main2", "([LString;D;I;)V'", 10);
            ReqMatch rmatch = new ReqMatcherBuilder().className("A").packageName("toto").arg("[LString").arg("D").arg("I").methodName("main").line(10).commit("1").build();
            Assertions.assertFalse(rmatch.isMatch(ste));
        }

        {
            StackTraceElement ste1 = new StackTraceElement("A.java", "toto", "toto.A", "main", "([LString;D;J;)V'", 10);
