...
class TestDiffGumtree {
    ...
    public void testPackageName() throws IOException {
        CommitFileMaterialization f1 = new CommitFileMaterialization(Paths.get("src/test/resources/B1.java"),
                "", "");
        CommitFileMaterialization f2 = new CommitFileMaterialization(Paths.get("src/test/resources/B2.java"),
                "commit2", "000002");
        Run.initGenerators();
        Diff.DiffBuilder builder = Diff.getBuilder();
        builder.add(f1.file, f1.issueId, f1.commitId);
