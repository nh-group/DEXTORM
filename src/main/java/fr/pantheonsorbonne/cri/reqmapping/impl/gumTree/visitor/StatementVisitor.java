    ...
    public void startTree(Tree tree) {
        ...
        */
        ...
        tree.getChildren().stream().filter(MethodDeclarationVisitor::isAStatement).forEach(t -> {
            this.getMatchersBuilders().add(this.parentMatcherBuilder.getCopy().commits((Collection<String>) (t.getMetadata(GumTreeFacade.BLAME_ID))).line(t.getLine()).pos(t.getPos()).len(t.getLength()));
